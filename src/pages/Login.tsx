...
import { useMinistries } from "@/hooks/use-dashboard-api";
...
const ROLES: { role: Role; icon: any; needsMinistry: boolean; hint: string }[] = [
  ...
  { role: "drejtor_inovacioni_publik", icon: Building2, needsMinistry: false, hint: "Të njëjtat opsione si drejtori i agjencisë" },
...
export default function Login() {
  ...
  const handleRolePick = async (r: typeof ROLES[number]) => {
    setError(null);
    if (r.needsMinistry) {
      setSelectedRole(r);
      setStep("ministry");
    ...
    await doLogin(r.role, undefined);
  };

  const doLogin = async (role: Role, ministry: string | undefined) => {
    setIsSubmitting(true);
    setError(null);

    try {
      const user = await api.login({ role, ministry, name: ROLE_LABEL[role] });
      login(user);
      navigate("/");
    } catch (err) {
    ...
    <div className="min-h-screen grid lg:grid-cols-2 bg-background">
      <div className="relative hidden lg:flex flex-col justify-between p-12 bg-gradient-surface border-r border-border overflow-hidden">
        ...
        <div className="relative space-y-6 max-w-lg">
          ...
          <h1 className="font-display text-5xl font-medium leading-[1.05] text-balance">Platforma e <span className="italic text-accent">Menaxhimit të Projekteve</span>.</h1>
          <p className="text-muted-foreground leading-relaxed">Një pamje e vetme për të gjitha projektet shtetërore: progres, OKR, risk dhe afate.</p>
        </div>

        <div className="relative text-xs text-muted-foreground font-mono flex items-center gap-2">
          <ShieldCheck className="size-3.5 text-success" />
          Akses i kontrolluar · Sesion i enkriptuar · backend aktiv
        </div>
      </div>
      ...
      <div className="flex items-center justify-center p-8 lg:p-12">
        <div className="w-full max-w-md space-y-6">
            ...
            <>
              ...
              <div className="space-y-2">
                {ROLES.map((r) => {
                  const Icon = r.icon;
                  return (
                    <button key={r.role} onClick={() => void handleRolePick(r)} disabled={isSubmitting} className="group w-full text-left p-4 rounded-md border border-border bg-surface hover:bg-surface-hover hover:border-border-strong transition-all flex items-center gap-4 disabled:opacity-60">
                      <div className={cn("size-10 rounded-md grid place-items-center shrink-0", r.role === "kryeminister" ? "bg-gradient-accent text-accent-foreground shadow-glow" : "bg-surface-elevated text-accent border border-border-strong")}>
          ...
          {step === "ministry" && selectedRole && (
            <>
              <button onClick={() => { setStep("role"); setSelectedRole(null); setError(null); }} className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-accent transition-colors">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">Po hyn si <span className="text-accent">{ROLE_LABEL[selectedRole.role]}</span>. Do të shohësh vetëm projektet e ministrisë së zgjedhur.</p>
              </div>

              <div className="space-y-2 max-h-[480px] overflow-y-auto pr-1">
                {ministries.map((m) => (
                  <button key={m} onClick={() => void doLogin(selectedRole.role, m)} disabled={isSubmitting} className="group w-full text-left p-3.5 rounded-md border border-border bg-surface hover:bg-surface-hover hover:border-accent/50 transition-all flex items-center gap-3 disabled:opacity-60">
