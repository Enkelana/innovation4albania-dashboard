...
import type { CreateProjectPayload, OKR, ProjectStatus, RiskLevel } from "@/types";
    ...
    <div className="p-6 lg:p-8 space-y-5 max-w-[1600px] mx-auto animate-fade-in">
      <div className="flex items-end justify-between gap-4 flex-wrap animate-slide-in-down">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Filtër kërkimi, 6 statuse dhe krijim projekti me ndërtues OKR.</p>
        </div>
      ...
      {showCreate && (user?.role === "drejtor_agjencie" || user?.role === "drejtor_inovacioni_publik") && (
        <div className="rounded-lg border border-border bg-surface p-5 shadow-elev space-y-4 animate-scale-in hover:shadow-glow transition-all duration-300">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Plotëso fushat bazë dhe shto objektivat me KR-të përkatëse.</p>
          </div>
          ...
          <Field label="Ekipi (ndarë me presje)">
            <Input value={form.team.join(", ")} onChange={(e) => setForm({ ...form, team: e.target.value.split(",").map((item) => item.trim()) })} />
          </Field>
