...
import type { Project, WorkgroupMember } from "@/types";
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!isLoading && projects.map((p, index) => {
              const teamMembers: WorkgroupMember[] = Array.isArray(p.teamMembers)
                ? p.teamMembers
                : (p.team ?? []).map((name, memberIndex) => ({
                    id: `${p.id}-fallback-${memberIndex}`,
                    name,
              ...
              const priorityLabel = p.priorityLabel ?? "Pa prioritet";
              const sectorLabel = p.sectorLabel ?? "Pa sektor";
              const cadenceDays = typeof p.updateCadenceDays === "number" ? p.updateCadenceDays : 14;
              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex flex-wrap gap-1.5">
                          <span className="rounded-full border border-border bg-background px-2 py-0.5 text-[10px] font-mono uppercase tracking-wider">{priorityLabel}</span>
                          <span className="rounded-full border border-border bg-background px-2 py-0.5 text-[10px] font-mono uppercase tracking-wider">{sectorLabel}</span>
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-3 py-4">
                    ...
                    <div className="text-[11px] text-muted-foreground mt-1">Update: çdo {cadenceDays} ditë</div>
                  </td>
                    ...
                    <div className="space-y-1">
                      <div className="flex -space-x-1.5 group-hover:gap-1 transition-all">
                        {teamMembers.slice(0, 3).map((member) => (
                          <div key={member.id} className="size-6 rounded-full bg-surface-elevated border border-border-strong text-[10px] grid place-items-center text-muted-foreground font-medium hover:scale-110 transition-transform hover:z-10 cursor-pointer" title={`${member.name} · ${member.roleLabel}`}>
                            {member.name.split(" ").map((n) => n[0]).slice(0, 2).join("")}
                          </div>
                        ))}
                        {teamMembers.length > 3 && <div className="size-6 rounded-full bg-surface-elevated border border-border-strong text-[10px] grid place-items-center text-muted-foreground hover:scale-110 transition-transform">+{teamMembers.length - 3}</div>}
                      </div>
                      <div className="text-[11px] text-muted-foreground">{teamMembers.length} role</div>
                    </div>
                  </td>
                  <td className="px-5 py-4">
